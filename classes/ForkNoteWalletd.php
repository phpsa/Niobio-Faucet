...
/**
 * RPC Wallet Class to handle transactions on a forknote wallet via RPC
 */
class ForkNoteWalletd
{

    /**
     * Server RPC Daeemon Url:port
     *
     * @var string Url to the deamon rpc instance
     */
    private $server;

    public function __construct($server = "http://127.0.0.1:8080")
    {
        $this->server = $server;
    }

    /**
     * Perform the call to the rpc instance
     *
     * @param array $req the request we are making
    ...
    private function apiCall($req)
    {
        ...
        if (!isset($result['result'])) {
            $msg = "API call to '" . $req['method'] . "' returned ";
            if (isset($result['error'])) {
                $msg .= "Error(" . $result['error']['code'] . "): " . $result['error']['message'] . PHP_EOL;
            } else {
                $msg .= "Unknown Error: " . print_r($result, true) . PHP_EOL;
            }
     ...
     * }
     *
     * @param string $publicSpendKey
     * @param string $secretSpendKey
     * @return void
     */
    public function createAddress($publicSpendKey = '', $secretSpendKey = '')
    {
        $args            = array();
        $args["jsonrpc"] = "2.0";
        $args["id"]      = "test";
     ...
     * @return void
     */
    public function getTransactionHashes($firstBlockIndex = 0, $firstblockHash = '', $blockCount = 20, $paymentId = '', $addresses = array())
    {
        $args            = array();
        $args["jsonrpc"] = "2.0";
        $args["id"]      = "test";
     ...
     * @return void
     */
    public function getTransactions($firstBlockIndex = false, $firstblockHash = false, $blockCount = 20, $paymentId = false, $addresses = false)
    {
        $args            = array();
        $args["jsonrpc"] = "2.0";
        $args["id"]      = "test";
     ...
     * @return void
     */
    public function getUnconfirmedTransactionHashes($addresses = array())
    {
        $args            = array();
        $args["jsonrpc"] = "2.0";
        $args["id"]      = "test";
     ...
     *
     *
     * @param array $fromAddresses Array of strings, where each string is an address to take the funds from
     * @param array $transfers array of Transaction Objects
     * @param string $paymentId if required
     * @param integer $anonymity Privacy level (a discrete number from 1 to infinity). Level 6 and higher is recommended.
     * @param integer $fee Transaction fee
     * @param string $changeAddress Valid and existing in this container address
     * @param int $unlockTime Height of the block until which transaction is going to be locked for spending.
     * @param string $extra String of variable length. Can contain A-Z, 0-9 characters.
     * @return void
     */
    public function sendTransaction($fromAddresses = false, $transfers, $paymentId = false, $anonymity = 6, $fee = 1000000, $changeAddress = false, $unlockTime = false, $extra = false)
    {
        ...
        //echo '<pre>$args: '; print_r($args); echo '</pre>'; die();
        $result = $this->apiCall($args);
        if ($result) {
            return $result['result'];
        }
    ...
    public function decimalToBigInt($value, $decimal_count = 10){
        return $value * (10 ** $decimal_count);
    }

    public function bigIntToDecimal($value, $decimal_count = 10){
        return $value / (10 ** $decimal_count);
    }
